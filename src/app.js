...
app.post("/add/todo", async(req, res) => {
  ...
  app.get("/edit/todo/:_id", async (req, res) => {
    try {
      let id = req.params._id;
      const todo = await Todo.findById(id);
      if (todo) {
        // Extract necessary properties to a plain object
        const plainTodo = {
          _id: todo._id,
          email: todo.email,
          todo: todo.todo,
        };
        // Convert the plain object to a JSON string and encode it for the URL
        const dataString = encodeURIComponent(JSON.stringify(plainTodo));
        // Redirect with the encoded data as a query parameter
        res.redirect(`/edit_users?data=${dataString}`);
      } else {
        res.redirect("/index");
      }
    } catch (err) {
  ...
  app.get("/edit_users", async(req,res)=>{
    const dataString = req.query.data;
    const todo = JSON.parse(decodeURIComponent(dataString));
    // Render your view and pass the data to the template
    res.render("edit_users", { todo: todo });
  });

  app.post("/update/todo/:_id", async (req, res) => {
    const { _id } = req.params;
    const { todo, regmail } = req.body;
